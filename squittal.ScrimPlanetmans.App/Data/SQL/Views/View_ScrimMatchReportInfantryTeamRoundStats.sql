...
FROM 
	(
		...
		FROM
			[PlanetmansDbContext].[dbo].ScrimMatchParticipatingPlayer match_players
		GROUP BY match_players.ScrimMatchId, match_players.TeamOrdinal
	) match_teams
INNER JOIN 
	[PlanetmansDbContext].[dbo].ScrimMatchRoundConfiguration match_rounds ON match_teams.ScrimMatchId = match_rounds.ScrimMatchId
LEFT OUTER JOIN (
	SELECT
		match_teams.ScrimMatchId,
		...
		SUM( CASE WHEN kills.AttackerTeamOrdinal = match_teams.TeamOrdinal THEN kills.Points
			WHEN kills.VictimTeamOrdinal = match_teams.TeamOrdinal AND DeathType = 0 THEN kills.Points * -1 ELSE 0 END
		) NetScore, 
		...
		SUM( CASE WHEN kills.VictimTeamOrdinal = match_teams.TeamOrdinal
			...
			THEN CASE WHEN kills.NextEventTimeDiff >= 9 AND kills.PrevEventTimeDiff > 6 THEN 1 ELSE 0 END
			WHEN kills.NextEventTimeDiff >= 6 AND kills.PrevEventTimeDiff > 3 THEN 1 ELSE 0 END
			ELSE 0 END
		) TrickleDeaths_6s
	FROM
		(
			SELECT
				match_teams.ScrimMatchId, match_teams.TeamOrdinal
			FROM
				[PlanetmansDbContext].[dbo].ScrimMatchParticipatingPlayer match_teams
			GROUP BY
				match_teams.ScrimMatchId, match_teams.TeamOrdinal
		) match_teams
		LEFT OUTER JOIN (
			...
			FROM 
				[PlanetmansDbContext].[dbo].ScrimDeath
		) kills ON match_teams.ScrimMatchId = kills.ScrimMatchId
			AND (match_teams.TeamOrdinal = kills.AttackerTeamOrdinal OR match_teams.TeamOrdinal = kills.VictimTeamOrdinal)
		LEFT OUTER JOIN (
			SELECT
				ScrimMatchId, damages.AttackerTeamOrdinal, VictimCharacterId, damages.Timestamp,
				COUNT(*) TotalDamages,
			...
			FROM 
				[PlanetmansDbContext].[dbo].View_ScrimDamageAssistDamageDealt damages
			GROUP BY
				ScrimMatchId, AttackerTeamOrdinal, Timestamp, VictimCharacterId
		) damage_sums ON kills.ScrimMatchId = damage_sums.ScrimMatchId
		...
		LEFT OUTER JOIN (
			SELECT 
				ScrimMatchId, grenades.AttackerTeamOrdinal, VictimCharacterId, grenades.Timestamp, COUNT(*) TotalGrenades,
			...
			FROM
				[PlanetmansDbContext].[dbo].[ScrimGrenadeAssist] grenades
			GROUP BY
				ScrimMatchId, AttackerTeamOrdinal, Timestamp, VictimCharacterId
		) grenade_sums ON kills.ScrimMatchId = grenade_sums.ScrimMatchId
		...
		LEFT OUTER JOIN (
			SELECT
				ScrimMatchId, spots.SpotterTeamOrdinal, VictimCharacterId, spots.Timestamp, COUNT(*) TotalSpots
			FROM
				[PlanetmansDbContext].[dbo].[ScrimSpotAssist] spots
			GROUP BY
				ScrimMatchId, SpotterTeamOrdinal, Timestamp, VictimCharacterId 
		) spot_sums ON kills.ScrimMatchId = spot_sums.ScrimMatchId
			...
			GROUP BY
				match_teams.ScrimMatchId, kills.ScrimMatchRound, match_teams.TeamOrdinal
	) kill_sums ON match_teams.ScrimMatchId = kill_sums.ScrimMatchId
...
LEFT OUTER JOIN (
	...
	GROUP BY match_players.ScrimMatchId, damages.ScrimMatchRound, match_players.TeamOrdinal
) damage_sums ON match_teams.ScrimMatchId = damage_sums.ScrimMatchId 
...
LEFT OUTER JOIN (
	...
	FROM
		[PlanetmansDbContext].[dbo].ScrimMatchParticipatingPlayer match_players
	INNER JOIN [PlanetmansDbContext].[dbo].ScrimGrenadeAssist grenades ON match_players.ScrimMatchId = grenades.ScrimMatchId
			AND match_players.CharacterId = grenades.AttackerCharacterId
	GROUP BY match_players.ScrimMatchId, grenades.ScrimMatchRound, match_players.TeamOrdinal
) grenade_sums ON match_teams.ScrimMatchId = grenade_sums.ScrimMatchId
...
LEFT OUTER JOIN (
	...
	FROM
		[PlanetmansDbContext].[dbo].ScrimMatchParticipatingPlayer match_players
	INNER JOIN [PlanetmansDbContext].[dbo].ScrimSpotAssist spots ON match_players.ScrimMatchId = spots.ScrimMatchId
			AND match_players.CharacterId = spots.SpotterCharacterId
	GROUP BY match_players.ScrimMatchId, spots.ScrimMatchRound, match_players.TeamOrdinal
) spot_sums ON match_teams.ScrimMatchId = spot_sums.ScrimMatchId 
		AND match_teams.TeamOrdinal = spot_sums.TeamOrdinal
		AND match_rounds.ScrimMatchRound = spot_sums.ScrimMatchRound
LEFT OUTER JOIN (
	SELECT 
		captures.ScrimMatchId,
		captures.ScrimMatchRound,
		captures.ControllingTeamOrdinal TeamOrdinal,
		SUM(captures.Points) Points
	FROM
		[PlanetmansDbContext].[dbo].ScrimFacilityControl captures
	GROUP BY captures.ScrimMatchId, captures.ScrimMatchRound, captures.ControllingTeamOrdinal
) capture_sums ON match_teams.ScrimMatchId = capture_sums.ScrimMatchId
